...
trait VoyagerCommon
{
    ...
    public function authorize( $ability, $arguments = [] ){
        return $this->authorizeForUser(auth('api')->user(), $ability, $arguments);
    }

    public function solveRelation( Builder &$query, DataType $dataType ){
        if( $dataType->rows->where('type', 'relationship')->isNotEmpty() ){
            $relations = $dataType->rows->where('type', 'relationship');

            foreach($relations as $relation){
                //dd($relation);
                $query->with(camelCase($relation->display_name));
            }
        }
    }
}
